...
# Token stats
output_tokens=$(jq -r '.modelUsage["claude-opus-4-8"].outputTokens // 0' "$STATS_FILE")
input_tokens=$(jq -r '.modelUsage["claude-opus-4-8"].inputTokens // 0' "$STATS_FILE")
cache_read=$(jq -r '.modelUsage["claude-opus-4-8"].cacheReadInputTokens // 0' "$STATS_FILE")
