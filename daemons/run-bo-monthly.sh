#!/usr/bin/env bash
# run-bo-monthly.sh — Monthly Bayesian optimization refinement daemon
# Fits GP surrogate, proposes candidates, validates, and generates report.
...
n_obs = bo.fit(lookback_days=30)
print(f'Fitted on {n_obs} observations')
if n_obs > 0:
    candidates = bo.propose(n_candidates=3)
    print(f'Proposed {len(candidates)} candidates')
    for i, c in enumerate(candidates):
        result = bo.validate(c)
        print(f'Candidate {i}: {result}')
    report_path = bo.generate_report()
    print(f'Report: {report_path}')
else:
    print('No observations, skipping proposal')
"
EXIT_CODE=$?
